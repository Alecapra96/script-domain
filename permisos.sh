...
$DIALOG --backtitle "Creado por : linkedin.com/in/alejandro-capra" \
	--title "PERMISOS DEL DOMINIO" --clear \
        --radiolist "Selecciona el permiso que deseas" 40 125 10 \
...
case $retval in
  0)
    ...
    case $choice in
     1)echo "Denegar login a todos los usuarios y permitir login de un usuario en particular"
         nombreUsuario=`$fundialog --stdout --title "nombre" --inputbox "Escribe el nuevo nombre del usuario:" 0 0`

     sudo realm deny -vR $nombreDominio -a
     sudo realm permit -vR $nombreDominio $nombreUsuario

     ;;
     2)echo "Denegar login a todos los usuarios y permitir login de un grupo en particular"
         nombreGrupo=`$fundialog --stdout --title "nombre" --inputbox "Escribe el nuevo nombre del grupo mas el @dominio.local" 0 0`

     sudo realm deny -vR $nombreDominio -a
     sudo realm permit -vR $nombreDominio -g  $nombreGrupo

     ;;
     3)echo "Permitir login a todos los usuarios del realm/Dominio"
     sudo realm permit -vR $nombreDominio -a

     ;;
     4)echo "Permitir login a un usuario particular del realm/Dominio"
         nombreUsuario=`$fundialog --stdout --title "nombre" --inputbox "Escribe el nuevo nombre del usuario:" 0 0`

     sudo realm permit -vR $nombreDominio $nombreUsuario

     ;;
     5)echo "Denegar login a todos los usuarios del realm/Dominio"
     sudo realm deny -vR $nombreDominio -a

     ;;
     6)echo "Permitir login a los miembros de un Grupo"
         nombreGrupo=`$fundialog --stdout --title "nombre" --inputbox "Escribe el nuevo nombre del grupo mas el @dominio.local" 0 0`

     sudo realm permit -vR $nombreDominio -g $nombreGrupo

     ;;
     7)echo "Denegar login a los miembros de un Grupo ya agregado"
         nombreGrupo=`$fundialog --stdout --title "nombre" --inputbox "Escribe el nuevo nombre del grupo mas el @dominio.local" 0 0`
